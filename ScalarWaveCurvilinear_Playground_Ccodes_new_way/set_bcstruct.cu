...
#define COUNT_INNER_OR_OUTER if(bc_gz_map[IDX3S(i0,i1,i2)].i0==-1) { num_ob_pts++;} else { num_ib_pts++; }
      ...
      cudaMallocManaged((void **)&(bcstruct->outer[which_gz]), sizeof(outer_bc)*num_ob_pts, cudaMemAttachGlobal);
      ...
      cudaMallocManaged((void **)&(bcstruct->inner[which_gz]), sizeof(inner_bc)*num_ib_pts, cudaMemAttachGlobal);
